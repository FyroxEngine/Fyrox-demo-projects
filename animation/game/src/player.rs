use fyrox::{
    ...
    scene::{animation::absm::prelude::*, pivot::Pivot, rigidbody::RigidBody},
...
#[visit(optional)]
pub struct Player {
    camera_pivot: InheritableVariable<Handle<Pivot>>,
    camera_hinge: InheritableVariable<Handle<Pivot>>,
    state_machine: InheritableVariable<Handle<AnimationBlendingStateMachine>>,
    model_pivot: InheritableVariable<Handle<Pivot>>,
    model: InheritableVariable<Handle<Pivot>>,
...
impl ScriptTrait for Player {
    ...
    fn on_update(&mut self, ctx: &mut ScriptContext) -> GameResult {
        ...
        let camera_hinge = ctx.scene.graph.try_get_mut(*self.camera_hinge)?;
        camera_hinge
            .local_transform_mut()
            .set_rotation(UnitQuaternion::from_axis_angle(
                &Vector3::x_axis(),
